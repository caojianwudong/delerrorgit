...
//## Module: CELLayer%375BECF903A3; Package specification
...
//	b. 图层描述信息存放在CELLayerInfo类中，通过GetELLayerInfoPtr()获取CELLayerInfo类的指针；
//	c. 图层属性数据的字段结构信息存放在CFieldsInfo类中，通过GetFieldsInfoPtr()获取CFieldsInfo类的指针；
//	d. 图层建立与删除：使用CreateELLayerInFile()在数据库中建立该图层对象，通过DeleteELLayerInFile()在数据库中删除该图层对象；
//	e. 属性信息处理：使用LoadELLayerInfo()从数据库中装载该图层的描述信息和属性数据的字段结构信息；如果对图层的部分描述信息进行了修改（图层名称不能修改），则使用UpdateELLayerInfoToFile()将修改后的内容保存到数据库中；
...
//	   该对象集合被创建后，如果是临时使用的集合对象，则可以由创建它的对象进行管理和删除。但如果该集合在创建后需要进行统一的管理，以便于进行后继的操作（如显示、查询/分析结果的联合等），则可以将该对象加入到属于CELLayer对象的CFeatureSets对象中，并通过该对象对所有被管理的特征对象集合进行访问。
...
class __declspec(dllexport) CELLayer: public CLayer  //## Inherits: <unnamed>%37AA4F38006F
{
  public:
    //## Constructors (specified)
      //## Operation: CELLayer%925876938
	//##ModelId=46B7B83A00DC
      CELLayer (CNaviMap* ownerNaviMap, LAYER layerID, jmeta * pFieldsInfo);

    //## Destructor (specified)
      //## Operation: ~CELLayer%928790182
    ...
    //## Other Operations (specified)
      //## Operation: SetELLayerType%933908409
	//##ModelId=455025290098
      //void SetELLayerType (EELLayerType ELLayerType);

      //## Operation: GetELLayerType%933908408
