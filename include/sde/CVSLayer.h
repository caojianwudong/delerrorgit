...
//## Module: CVSLayer%375BECF903A3; Package specification
...
//	b. 图层描述信息存放在CVSLayerInfo类中，通过GetPOILayerInfoPtr()获取CVSLayerInfo类的指针；
...
//	   该对象集合被创建后，如果是临时使用的集合对象，则可以由创建它的对象进行管理和删除。但如果该集合在创建后需要进行统一的管理，以便于进行后继的操作（如显示、查询/分析结果的联合等），则可以将该对象加入到属于CVSLayer对象的CFeatureSets对象中，并通过该对象对所有被管理的特征对象集合进行访问。
...
//##ModelId=46B7B8360320
class __declspec(dllexport) CVSLayer: public CLayer  //## Inherits: <unnamed>%37AA4F38006F
{ 
  public:
	//##ModelId=46B7B836032C
	  CFeature * NewArcFeature ( );
    //## Constructors (specified)
      //## Operation: CVSLayer%925876938
	//##ModelId=46B7B836032D
      CVSLayer (CNaviMap* ownerNaviMap, LAYER LayerID, jmeta * pFieldsInfo);

    //## Destructor (specified)
      //## Operation: ~CVSLayer%928790182
	//##ModelId=46B7B8360331
      ~CVSLayer ();
