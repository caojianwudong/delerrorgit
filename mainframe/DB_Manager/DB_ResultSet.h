...
public:
	...
	//保存一条数据到结果集中
	void SetResValue(std::vector<std::string> row_value);
	void SetFieldType(int type);
	void SetFieldName(const char* sName);

private:
	std::vector<std::vector<std::string> > m_vtValue;//当前结果集所有行列值
	std::vector<int> m_vtFieldType;//当前结果集属性类型
	std::vector<std::string> m_vtFieldName;//当前结果集属性名称
